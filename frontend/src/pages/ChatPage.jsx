...
import { asyncFetchUserChats, asyncCreateNewChat, asyncUpdateChatTitle } from '../store/services/chatService';
import { setActiveChat } from '../store/slices/chatSlice';
import { messageAdd } from '../store/slices/messageSlice'; // <-- CORRECT IMPORT
...
const LogoutIcon = () => (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9 21H5C4.46957 21 3.96086 20.7893 3.58579 20.4142C3.21071 20.0391 3 19.5304 3 19V5C3 4.46957 3.21071 3.96086 3.58579 3.58579C3.96086 3.21071 4.46957 3 5 3H9" stroke="#9ca3af" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /><path d="M16 17L21 12L16 7" stroke="#9ca3af" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /><path d="M21 12H9" stroke="#9ca3af" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>);
const CodeBlock = ({ code, language }) => {
  const [copied, setCopied] = useState(false);
  const copyToClipboard = () => { navigator.clipboard.writeText(code).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2500); }); };
  return (<div className="code-block-wrapper"><div className="code-block-header"><span>{language || 'code'}</span><button onClick={copyToClipboard} className="copy-button">{copied ? 'Copied!' : 'Copy'}</button></div><SyntaxHighlighter language={language} style={vscDarkPlus} customStyle={{ margin: 0, padding: '1rem', borderBottomLeftRadius: '0.5rem', borderBottomRightRadius: '0.5rem' }} codeTagProps={{ style: { fontFamily: '"Fira Code", "Fira Mono", monospace', fontSize: '0.9rem' } }} wrapLongLines={true}>{code}</SyntaxHighlighter></div>);
};
...
const ChatPage = () => {
  ...
  const chatHistory = Object.values(chatsById).sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
  ...
  useEffect(() => {
    ...
    socket.current.on("response", (data) => {
      const aiMessage = { _id: `bot-${Date.now()}`, sender: "bot", text: typeof data === "string" ? data : data?.content || JSON.stringify(data) };
  ...
  useEffect(() => { if (textareaRef.current) { textareaRef.current.style.height = 'auto'; textareaRef.current.style.height = `${textareaRef.current.scrollHeight}px`; } }, [userInput]);
  ...
  const handleSendMessage = (e) => {
    ...
    if (activeChat.title === "New Chat" && messages.length === 0) {
      const newTitle = userInput.trim().slice(0, 40) + (userInput.trim().length > 40 ? '...' : '');
      dispatch(asyncUpdateChatTitle(activeChatId, newTitle));
    }

    const userMessage = { _id: `user-${Date.now()}`, sender: "user", text: userInput };
      ...
      <div className="chat-page">
        <aside className={`chat-sidebar ${sidebarOpen ? 'open' : ''}`}>
          <div className="sidebar-content">
            <button className="new-chat-button" onClick={handleNewChat}><PlusIcon /> New Chat</button>
            <div className="chat-history">
              <h3 className="history-title">Recent</h3>
              {chatHistory.map(chat => (<div key={chat._id} className={`chat-history-item ${chat._id === activeChatId ? 'active' : ''}`} onClick={() => selectChat(chat._id)}>{chat.title}</div>))}
            </div>
            <div className="sidebar-footer"><div className="user-profile"><div className="user-avatar"><UserIcon /></div><span>{user.name}</span></div></div>
          </div>
        </aside>
        <div className="sidebar-backdrop" onClick={() => setSidebarOpen(false)}></div>
        <main className="chat-main">
          <header className="chat-header">
            <button className="menu-button" onClick={() => setSidebarOpen(!sidebarOpen)}><MenuIcon /></button>
            <div className="header-title"><NexieIcon size={28} /><h1>Nexie</h1></div>
            <button className="logout-button" onClick={handleLogout} title="Logout"><LogoutIcon /></button>
          </header>
          <div className="chat-log-wrapper">
            <div className="chat-log" ref={chatLogRef}>
              {messages.length === 0 && !isLoading && (<div className="welcome-message"><NexieIcon size={48} /><h2>Hey {user.name}, how can I help you today?</h2></div>)}
              {messages.map((msg) => (<div key={msg._id} className={`message-wrapper ${msg.sender === "user" ? "user-wrapper" : "bot-wrapper"}`}><div className={`message-bubble ${msg.sender === "user" ? "user-bubble" : "bot-bubble"}`}><div className="message-content"><ReactMarkdown children={String(msg.text ?? "")} components={{ code(props) { const { children, className } = props; const match = /language-(\w+)/.exec(className || ''); return match ? (<CodeBlock language={match[1]} code={String(children).replace(/\n$/, '')} />) : (<code className={className}>{children}</code>) } }} /></div></div></div>))}
              {isLoading && (<div className="message-wrapper bot-wrapper"><div className="message-bubble bot-bubble"><div className="message-content thinking-indicator"><span /><span /><span /></div></div></div>)}
            </div>
          </div>
           <div className="chat-input-area"><div className="chat-input-container"><form onSubmit={handleSendMessage} className="chat-input-form"><textarea ref={textareaRef} value={userInput} onChange={(e) => setUserInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) handleSendMessage(e); }} placeholder="Message Nexie..." rows="1" /><button type="submit" className="send-button" disabled={!userInput.trim() || isLoading}><SendIcon /></button></form></div><p className="disclaimer">Nexie may produce inaccurate information. Please verify important details.</p></div>
        </main>
      </div>
    </div>
  );
